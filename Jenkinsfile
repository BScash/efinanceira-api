...
pipeline {
    agent any
    parameters {
            booleanParam(name: 'Sonar-off', defaultValue: false, description: 'Desativar o Sonar ?')
    ...
    stages {
        ...
        stage('Build application') {
            steps {
                script {
            target_file = utils.newMavenInstallCleanDependencies('/opt/maven-3.8.8',artifactId, artifactVersion,finalName, packaging,true, false, params['Limpar dependências'])
